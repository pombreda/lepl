...
sed -i -r 's/``(SignedFloat|Literal|Drop|Star|Space)\(\)``/`\1() <api\/redirect.html#lepl.functions.\1>`_/g' *.rst
...
sed -i -r 's/matchers\.(SignedFloat|Literal|Drop|Star|Space)/functions.\1/g' *.rst
